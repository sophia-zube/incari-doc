...
There are two uniform variables that can be used in the configuration of both *shaders*:

* `incTime`: **Float** that keeps the time from the first rendering of the **Scene**.
...
* `fragCoord`: 2-vector that contains the 2-dimensional coordinates of the pixel.

And the output is:

* `fragColor`: 4-vector that stores the color that the pixel will be set to.
