...
|`Transformation`|The features under this **Attribute** are `Position`, `Rotation`, and `Scale`. Together, they can alter the size, location, and rotation of the **Plane Object** within 3D space.|
|`Rotation Pivot`|Sets the point of rotation.|
|`Plane`|`Rows` sets the number of rows, which segment the **Plane** horizontally. `Columns` sets the number of columns, which segment the **Plane** vertically.|
...
|`Rotation Pivot`|Sets the point of rotation.|
|`Torus`|`Rows` sets the number of rows, which segment the **Torus** horizontally. The higher the number, the smoother the surface. `Columns` sets the number of columns, which segment the **Torus** vertically. For example, if `Columns` were set to 4, then the number of sections would be 4 and the **Torus** would take a square-like shape. `Radius` increases or decreases the radius of the **Torus**. If one thinks of the **Torus** as many circular pieces patched together, it is the radius of these circles that are changed. This is visualized as the **Torus** becoming thicker or thinner. `Inner Radius` sets the radius of the internal space of the **Torus**.|
